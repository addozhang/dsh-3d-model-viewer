...
`;
  ...
  async function parse3mf(buf) {
    ...
    for (const f of models) {
      ...
      const emit = (mesh2, defaultColor, materialColors) => {
        const verts = [...mesh2.getElementsByTagNameNS("*", "vertex")].map((v) => [+v.getAttribute("x") * factor, +v.getAttribute("y") * factor, +v.getAttribute("z") * factor]);
      ...
      for (const obj of objects) {
        ...
        for (const mesh2 of obj.getElementsByTagNameNS("*", "mesh")) emit(mesh2, objColor, materialColors);
      }
      if (!objects.length) for (const mesh2 of doc.getElementsByTagNameNS("*", "mesh")) emit(mesh2, null, null);
    }
