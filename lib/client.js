...
`;
  ...
  var VS = `attribute vec3 p;attribute vec3 n;uniform mat4 mvp;uniform mat4 model;varying vec3 vn;void main(){gl_Position=mvp*vec4(p,1.);vn=mat3(model)*n;}`;
  var FS = `precision mediump float;varying vec3 vn;uniform vec3 uColor;void main(){vec3 N=normalize(vn);vec3 L=normalize(vec3(.5,.8,1.));float d=max(dot(N,L),0.);float rim=pow(1.-abs(N.z),2.);vec3 c=uColor*(.28+.72*d)+rim*vec3(.08,.18,.28);gl_FragColor=vec4(c,1.);}`;
  ...
  function orthographic(aspect) {
    const k = 1.15, near = 0.01, far = 100;
  ...
  function rayMesh(o, d, mesh, limitTriangles = 4e5) {
    if (!rayBox(o, d, mesh.lo, mesh.hi)) return false;
    if (mesh.triangles > limitTriangles) return true;
  ...
  function Viewer({ mesh, resetToken, view, color }) {
    ...
    return h("canvas", {
      ...
      tabIndex: 0,
      role: "img",
      "aria-label": "3D \u6A21\u578B\u89C6\u56FE\uFF1A\u62D6\u52A8\u6A21\u578B\u65CB\u8F6C\uFF0C\u62D6\u52A8\u80CC\u666F\u5E73\u79FB\uFF0C\u6EDA\u8F6E\u7F29\u653E",
  ...
  // src/ui.js
  var M3D_DOC_ID = "@local/dsh-3d-model-viewer/model3d";
  ...
  function Model3dBody({ content, resourceAddress, t }) {
    ...
    return h(
      ...
      state.status === "ready" && h("div", { className: "d3v-help" }, "\u62D6\u52A8\u6A21\u578B\u65CB\u8F6C \xB7 \u62D6\u52A8\u80CC\u666F\u5E73\u79FB \xB7 \u6EDA\u8F6E\u7F29\u653E \xB7 1-7 \u89C6\u89D2 R \u590D\u4F4D")
    );
  }
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          s.mesh && h("div", { className: "d3v-help" }, "\u62D6\u52A8\u6A21\u578B\u65CB\u8F6C \xB7 \u62D6\u52A8\u80CC\u666F\u5E73\u79FB \xB7 \u6EDA\u8F6E\u7F29\u653E \xB7 \u6587\u4EF6\u53D8\u5316\u65F6\u81EA\u52A8\u66F4\u65B0")
        ),
        h(
          "footer",
  ...
  function SessionBridge({ sessionId, sessions, stores, layoutStore }) {
    ...
    const window2 = React.useSyncExternalStore(binding.eventSource.subscribe, binding.eventSource.getSnapshot, binding.eventSource.getSnapshot);
    const s = useStore(sessionStore);
    React.useEffect(() => {
      let live = true, timer;
      const refresh = async () => {
  ...
  window.__D3V_DEBUG__ = { Viewer: viewer_exports, Parse: parse_exports };
})();
