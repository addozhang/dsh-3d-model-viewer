...
`;
  ...
  function ViewPresetBar({ onPick, color, onPickColor }) {
    return h(
      ...
      h("div", { className: "d3v-views" }, VIEW_PRESETS.map(([k, label, ang]) => h("button", {
        key: k,
        type: "button",
        className: "d3v-view-btn",
        title: label + (ang[2] ? "\uFF08\u6B63\u4EA4\uFF09" : ""),
  ...
  function Viewer({ mesh, resetToken, view, color }) {
    ...
    const orient = React.useRef({ yaw: 0.65, pitch: -0.45, ortho: false });
    ...
    React.useEffect(() => {
      ...
      const pm = (e) => {
        ...
        o.yaw += (e.clientX - lx) * 0.01;
        o.pitch = Math.max(-1.5, Math.min(1.5, o.pitch + (e.clientY - ly) * 0.01));
        o.ortho = false;
        lx = e.clientX;
        ly = e.clientY;
        request();
  ...
  function Model3dBody({ content, resourceAddress, t }) {
    ...
    return h(
      ...
      state.status === "ready" && h("div", { className: "d3v-help" }, "\u62D6\u52A8\u65CB\u8F6C \xB7 \u6EDA\u8F6E\u7F29\u653E \xB7 \u6B63\u4EA4\u89C6\u56FE\u663E\u793A\u5C3A\u5BF8")
    );
  }
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          s.mesh && h("div", { className: "d3v-help" }, "\u62D6\u52A8\u65CB\u8F6C \xB7 \u6EDA\u8F6E\u7F29\u653E \xB7 \u6587\u4EF6\u53D8\u5316\u65F6\u81EA\u52A8\u66F4\u65B0")
        ),
        h(
          "footer",
